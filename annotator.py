...
class Annotator():
    ...
    def __init__(self, video_dir, data_dir, scene_id=1):
        ...
        try:
            data_path = os.path.join(data_dir,"obj","scene{}_annotations.csv".format(scene_id))
            ts_path = os.path.join(data_dir,"ts","scene{}_ts.csv".format(scene_id))

            self.load_data_csv(data_path)
    ...
    def load_data_csv(self,csv_path):
        ...
        for line in lines:
            ...
            obj_id = "{}_{}".format(datum["id"],datum["camera"])
...
def _data_to_csv(data,scene_id):
    ...
    #note to self - do something verifiably repulsive and jam the class_dict into the class header so the information is stored in the csv (in a vile way) *shivers*
    ...
    for fidx,frame_data in enumerate(data):
        if fidx % 200 == 0:
            print("On frame {} for scene {}".format(fidx,scene_id))
            ...
            for key in header:
                try:
                    if key == "gen":
                        ...
                        try:
                            row.append(safe(np.round(safe(obj[key]),3)))
                        except:
                            row.append(safe(obj[key]))
    ...
    # write rows
    header = ["frame"] + header    
    outfile = "scene{}_annotations.csv".format(scene_id)
    with open(outfile,"w") as f:
        
        csvwriter = csv.writer(f) 
        csvwriter.writerow(header)
        csvwriter.writerows(all_rows)
    print("Wrote {} with {} points".format(outfile,len(all_rows)))

def _hg_to_csv(hg,poly_params,scene_id):
    
    """
    We need to store the correspondence attribute for each homography component of the HomographyWrapper class
    but first we need to add the relevant poly_params to each correspondence
    """
    import json
    from json import JSONEncoder
    ...
    print("Storing homography for scene {}".format(scene_id))
    
    for corr in hg.hg1.correspondence:
        key = corr + "_EB"
        pp = poly_params[key]
        hg.hg1.correspondence[corr]["curve"] = pp
    for corr in hg.hg2.correspondence:
        key = corr + "_WB"
        pp = poly_params[key]
        hg.hg2.correspondence[corr]["curve"] = pp
    
    main  =  {
                "EB":hg.hg1.correspondence,
                "WB":hg.hg2.correspondence
              }
    
    with open("scene{}_hg.json".format(scene_id), "w") as f:
        json.dump(main, f, indent = 6,cls = NumpyArrayEncoder)
    
def _ts_to_csv(ts,scene_id):
    headers = [key for key in ts[0].keys()]
    all_rows = []
    
    
    for fidx,frame_data in enumerate(ts):
        row =[fidx]
        for key in headers:
            try:
                row.append(np.round(frame_data[key],4))
            except:
                row.append("-1")
        all_rows.append(row)
    ...
    outfile = "scene{}_ts_orig.csv".format(scene_id)
    with open(outfile,"w") as f:
...
def _spldata_to_csv(data,timestamps,splines,scene_id,Hz = 30):
    import json
    from json import JSONEncoder
    ...
    for o_idx in range(len(splines)):
        ...
        # if splines[o_idx][0] is None or splines[o_idx][1] is None:
        #     no_spline_idxs.append(o_idx)
        #     continue
        ...
        for datum in data:
            ...
            if set_statics:
                obj_container["l"] = datum
                obj_container["gen"] = "manual"
            for obj in datum.keys():
                obj_id = int(obj.split("_")[-1])
                if obj_id == o_idx:
                    
                    if set_statics:
                        obj_container["l"] = safe(datum[obj]["l"])
                        obj_container["w"] = safe(datum[obj]["w"])
                        obj_container["h"] = safe(datum[obj]["h"])
                        obj_container["class"] = safe(datum[obj]["class"])
                        obj_container["id"] = safe(datum[obj]["id"])
                    ...
                    try:
                        obj_container["gen"] = [obj["gen"]]
                    except:
                        pass
        
                
        #sample spline every 30th of second between min and max time
        for ts in np.arange(min_ts,max_ts,1/Hz):
            if splines[o_idx][0] is not None:
                obj_container["timestamp"].append(safe(ts))
                        ...
                        for obj in datum.keys():
                            ...
                            if obj_id == o_idx:
                                if datum[obj]["timestamp"] <= ts and np.abs(ts-datum[obj]["timestamp"]) < 1/30:
                                    ...
                                    obj_container["x_position"].append(safe(datum[obj]["x"]))
                                    obj_container["y_position"].append(safe(datum[obj]["y"]))
                                    DONE = True
                                    break
        
        if len(obj_container["timestamp"]) > 0:
            obj_container["direction"] = 1 if obj_container["y_position"][0] < 60 else -1
            objects.append(obj_container)
        
        with open("scene{}_splobj.json".format(scene_id), "w") as f:
            json.dump(objects, f, indent = 6,cls = NumpyArrayEncoder)
